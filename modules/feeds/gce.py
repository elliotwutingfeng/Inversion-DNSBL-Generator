...
async def _get_googleusercontent_url_list() -> AsyncIterator[set[str]]:
    ...
    if resp == b"{}":
        logger.warning("Failed to retrieve Google Compute Engine IP ranges; returning empty list")
        yield set()
    ...
    ipv4Prefixes_and_scopes = (
        (x["ipv4Prefix"], x["scope"]) for x in prefixes if x.get("service", "") == "Google Cloud" and ("ipv4Prefix" in x) and ("scope" in x)
    )

    ip_ranges = _collapse_cidrs([x[0] for x in ipv4Prefixes_and_scopes])  # Removes overlapping ip ranges
    ...
    def __init__(self, parser_args: dict, update_time: int):
        ...
        if "gce" in parser_args["sources"]:
            ...
            if parser_args["fetch"]:
                ...
                self.jobs = [(_get_googleusercontent_url_list, update_time, db_filename) for db_filename in self.db_filenames]
